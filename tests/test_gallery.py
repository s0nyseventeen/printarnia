...
def test_check_image_true():
    assert check_image(Image()) is True


def test_check_image_false():
    assert check_image(Image('kit')) is False
...
def test_delete_work(app, auth, client):
    with app.app_context():
        db = get_db()
        db.run_query(INSERT_INTO_WORK)
        auth.register(
...
def test_update_image_dbrecord(app, auth, client):
    with app.app_context():
        db = get_db()
        auth.register(
            {'username': 'admin', 'password': 'admin', 'email': 'admin@mail.ua'}
...
def test_update_image_redirect(app, auth, client):
    with app.app_context():
        db = get_db()
        auth.register(
            {'username': 'admin', 'password': 'admin', 'email': 'admin@mail.ua'}
...
def test_create_without_title(app, auth, client):
    with app.app_context():
        auth.register(
            {'username': 'admin', 'password': 'admin', 'email': 'admin@mail.ua'}
        )
...
def test_create_with_image_dbrecord(app, auth, client):
    with app.app_context():
        auth.register(
            {'username': 'admin', 'password': 'admin', 'email': 'admin@mail.ua'}
        )
...
def test_index(app, client, expected):
    with app.app_context():
        db = get_db()
        db.run_query(INSERT_INTO_WORK)
